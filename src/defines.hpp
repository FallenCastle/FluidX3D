...
#define FP16S // legacy default; the configuration runner selects DdfStorage per instance. Optional for 2x speedup and 2x VRAM footprint reduction: compress LBM DDFs to range-shifted IEEE-754 FP16; number conversion is done in hardware; all arithmetic is still done in FP32
...
// Host support for SURFACE and TEMPERATURE is compiled into the configuration runner. The OpenCL program and
// allocations are selected per SolverOptions instance, so old single-phase configurations keep their original path.
#define SURFACE // runtime-selectable free-surface support
#define TEMPERATURE // runtime-selectable temperature support
...
//#define GRAPHICS // configuration runner uses console and VTK output only

#define GRAPHICS_FRAME_WIDTH 1280 // set frame width if only GRAPHICS is enabled
#define GRAPHICS_FRAME_HEIGHT 720 // set frame height if only GRAPHICS is enabled
...
// UPDATE_FIELDS is emitted into the per-instance OpenCL program when free-surface fields require it.
