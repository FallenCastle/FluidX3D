...
**V1.0.0 新 EXE 的完整发布验收已在 2026-09-27（北京时间）通过。** P1/P2/存储/P3/扫描五套脚本均退出 0，聚合 `status=succeeded`、`suite_status=succeeded`、`suites_skipped=false`、`package_unchanged=true`。P3 重新执行全部 16 模型组合和 96 个通道剖面，没有引用旧 EXE 的矩阵报告。包运行入口的正常周期流、Ahmed 零步预处理均通过。

被测 EXE SHA256：`706e1bd99c2e0d9cb326be828266df33b6979651b2f25e58d118ee6aed471a5c`。构建提交：`e8391259fd4249222297e49fd0336c5d6f2fa3a2`；C++ `src` 树：`dac334faf52865bf1fc5fe5fc5b6ed35b99b5ed8`。最终源码归档提交以发布包 `manifest.json` 为准，可能因后续文档与归档证据提交而不同于原始构建提交；应核对源码树、项目/构建身份及 EXE 哈希。

详细结果见 [V1.0.0 验收记录](validation/solver-ibm-v1.0.0-2026-09-27.md)。正式包中的证据入口为 `tests/reports/release/full-acceptance.json`、`tests/reports/release/suites/<name>/report.json`；最终候选包核验为 `tests/reports/release/package-verification.json`，接手时读取其实际状态，不能把候选目录的成功等同于最终封存位置已经核验。完整回归期间候选包未被修改。

## 已知数值和接口限制

本次 V1.0.0 重新验收与历史结论一致：48 组 FP32 通道达到既定速度/受力阈值；FP16S 的 48 组中有 32 组未达到相同阈值，弱强迫及细通道中量化误差明显。保留这一限制，不把运行成功等同于精度通过。历史三维剪切波曾在 24³ 网格略超过 3% 阈值，后续三网格细化显示误差下降，32³ 通过；初次失败报告与追加记录同时保留。
