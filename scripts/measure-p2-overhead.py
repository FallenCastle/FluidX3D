...
def main():
    ...
    config=json.loads((workspace/'src'/'configs'/'periodic-lattice.json').read_text())
    ...
    for label,build,analysis in [('p1','config-runner',False),('p2','config-runner-p2',False),('p2-sampled','config-runner-p2',True)]:
        ...
        exe=workspace/'bin'/build/'FluidX3D.exe'
        ...
        for i in range(3):
            ...
            with (case/'stdout.log').open('wb') as out,(case/'stderr.log').open('wb') as err:
                completed=subprocess.run([str(exe),'--config',str(case/'case.json'),'--output',str(case/'results'),'--device','0'],stdout=out,stderr=err,timeout=240)
